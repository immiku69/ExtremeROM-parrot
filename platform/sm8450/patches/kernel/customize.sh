...
REPLACE_KERNEL_BINARIES()
{
    ...
    ZIP_LINK="$EXTREMEKRNL_REPO"
    echo "Downloading kernel image: $(basename "$ZIP_LINK")"
    curl -L -s -o "$TMP_DIR/boot.img" "$ZIP_LINK"

    echo "Copying kernel binary"
    rm -f "$WORK_DIR/kernel/boot.img"
    cp "$TMP_DIR/boot.img" "$WORK_DIR/kernel/boot.img"
