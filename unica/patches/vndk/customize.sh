...
if [[ "$SOURCE_VNDK_VERSION" != "$TARGET_VNDK_VERSION" ]]; then
    ...
    if [ ! -f "$SYS_EXT_DIR/apex/com.android.vndk.v$TARGET_VNDK_VERSION.apex" ]; then
        ...
        case "$TARGET_VNDK_VERSION" in
            "30")
                ADD_TO_WORK_DIR "$SRC_DIR/prebuilts/a73xqxx" "system_ext" "apex/com.android.vndk.v30.apex" 0 0 644 "u:object_r:system_file:s0"
                ;;
            "31")
                ADD_TO_WORK_DIR "$SRC_DIR/prebuilts/r0qxxx" "system_ext" "apex/com.android.vndk.v31.apex" 0 0 644 "u:object_r:system_file:s0"
                ;;
            "33")
