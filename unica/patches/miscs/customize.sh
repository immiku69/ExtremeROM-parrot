...
if [[ -f "$TARGET_FIRMWARE_PATH/vendor/lib64/liblivefocus_capture_engine.so" ]]; then
    if grep -q "ro.product.name" "$TARGET_FIRMWARE_PATH/vendor/lib64/liblivefocus_capture_engine.so"; then
